...
// MMMMMMMM UUUUUUUU GGGGGGGGG XXXXXXXX YYYY-MM-DD HH:MM NAME (->LINK)
    ...
    case S_IFBLK:
    case S_IFCHR:
        printf("%3d %s %-8s %-8s %3d, %3d %s %s\n",
               s.st_mode, mode, user, group,
               (int) MAJOR(s.st_rdev), (int) MINOR(s.st_rdev),
               date, name);
        break;
    case S_IFREG:
        printf("%3d %s %-8s %-8s %8lld %s %s\n",
               s.st_mode, mode, user, group, s.st_size, date, name);
        break;
    case S_IFLNK: {
        char linkto[256];
        int len;
        ...
        printf("%3d %s %-8s %-8s          %s %s -> %s\n",
               s.st_mode, mode, user, group, date, name, linkto);
        break;
    }
    default:
        printf("%3d %s %-8s %-8s          %s %s\n",
               s.st_mode, mode, user, group, date, name);
